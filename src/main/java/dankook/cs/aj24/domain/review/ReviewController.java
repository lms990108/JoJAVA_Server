...
public class ReviewController {
    ...
    @GetMapping("/targetPlace")
    @Operation(summary = "특정 targetPlace에 대한 리뷰 조회", description = "특정 targetPlace에 대한 리뷰 목록을 반환합니다.")
    @ApiResponse(responseCode = "200", description = "리뷰 목록 조회 성공", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ReviewDocument.class)))
    public Page<ReviewDocument> getReviewsByTargetPlace(@RequestParam String targetPlaceId, @RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "10") int size) {
